...
expand_buses(Pin *curpin, BUStype *curbus, char *busformat)
{
	...
	if (i != low) {
	    ...
	    newpin->function = (curpin->function != NULL) ?
		    strdup(curpin->function) : NULL;
...
read_liberty(char *libfile, char *pattern)
{
	...
	switch (section) {
		...
		if (!strcasecmp(token, "}")) {
		    fprintf(stderr, "End of library at line %d\n", libCurrentLine);
		...
		if (!strcmp(token, "}")) {
			...
			if (newpin->function != NULL)
			    free(newpin->function);
			newpin->function = strdup(rfunc);
			if (newcell->function == NULL && newpin->type == PIN_OUTPUT)
			...
			if (newcell->function == NULL && newpin->function != NULL)
			    newcell->function = strdup(newpin->function);
		    }
		}
    ...
    fprintf(stderr, "Lib Read:  Processed %d lines.\n", libCurrentLine);
...
void delete_Cell(Cell *cell) {
    ...
    while (curpin != NULL) {
        tmppin = curpin->next;
        free(curpin->name);
        if (curpin->function != NULL) free(curpin->function);
        free(curpin);
        curpin = tmppin;
    }
    free(cell);
